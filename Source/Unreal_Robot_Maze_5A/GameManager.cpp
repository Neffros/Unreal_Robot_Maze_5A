...
void AGameManager::BeginExplorationPhase()
{
    ...
    this->_enabledCrossroadHints = 0;

    for (auto crossRoad : this->CrossRoadController->GetCrossRoads())
    {
        if (crossRoad->direction != DirectionEnum::None)
            ++this->_enabledCrossroadHints;
    }
...
void AGameManager::BeginEndPhase(bool isWin)
{
    ...
    if (isWin)
    {
        this->SetGameTimer(this->GetGameTimer() + this->_enabledCrossroadHints * 10);
